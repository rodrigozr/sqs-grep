...
4. Run `npm run integration-test && npm test` and make sure that all tests are passing and the code coverage is at 100%
   (the integration tests need Docker/Finch to start a [Floci](https://github.com/floci-io/floci) container, a free local AWS emulator; `npm run test:watch` re-runs the unit tests on every change)
